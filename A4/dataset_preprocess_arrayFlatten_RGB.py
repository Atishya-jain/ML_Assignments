...
for game in games_list[beg:end]:
	...
	for subsample in subsample_list:
		...
		for image in images_list:
			...
			vector = arr.reshape(3, shape[1], shape[0])
			if ct==0:
				to_add = vector
			else:
				to_add = np.append(to_add, vector, axis=0)
...
# pickle.dump(test, f)
# f.close()
np.save("train_x_"+str(beg)+"_"+str(end)+"_rgb", X_train)
np.save("test_x_"+str(beg)+"_"+str(end)+"_rgb", X_test)
np.save("train_y_"+str(beg)+"_"+str(end)+"_rgb", Y_train)
np.save("test_y_"+str(beg)+"_"+str(end)+"_rgb", Y_test)
